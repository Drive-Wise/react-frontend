         ...
         <div className={`flex lg:w-3/4 m-2 sm:m-4 md:m-6 lg:m-8 xl:m-10  flex-col-reverse lg:flex-row-reverse `}>
            ...
            <div className={`flex lg:w-1/2  items-center lg:${imageFlipStyle} `}>
                <div className="lg:w-3/4 lg:mx-10">
                    <header className="flex leading-tight my-3 m-auto text-title font-sans font-bold  text-4xl"><h1>{header_text}</h1></header>
                    <p className="flex my-3 m-auto text-textp font-sans text-md">{paragraph_text}</p>
                    <a href={button_link}><button className="m-auto my-3 bg-title font-semibold leading-6 text-textp text-sm rounded-lg px-4 py-2 transition duration-300 hover:bg-red-400">{button_text}</button> </a>
